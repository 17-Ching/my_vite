...
<template>
    ...
    <article class="task_container">
      ...
      <div class="task_add_block" ref="taskAddBlock"> <!--ref是參照-->
        <input 
            type="text" 
            class="task_name" 
            placeholder="輸入待辦事項…"
...
//Data
const taskText = ref(""); //{value: 元素}
const tasks = ref([]); //{nalue: 元素}
...
function taskRemove(e, i){  //(事件物件, 索引直)
              // console.log(i);
              let r = confirm("確認移除?");
              if (r){
                e.target.closest("li").classList.add("fade_out");
